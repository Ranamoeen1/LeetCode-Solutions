class Solution:
    def maximumAmount(self, coins: List[List[int]]) -> int:
        ...
        # dp[k][j] = max coins at current row, column j with k neutralizations used
        dp = [[-float('inf')] * n for _ in range(3)]
        
        # Initialize first cell
        dp[0][0] = coins[0][0]
        dp[1][0] = max(coins[0][0], 0)
        dp[2][0] = max(coins[0][0], 0)
        
        # Fill first row
        for j in range(1, n):
            val = coins[0][j]
            for k in range(3):
                # Come from left without neutralizing current
                dp[k][j] = dp[k][j-1] + val
                # If we can neutralize and val is negative, try neutralizing
                if k > 0 and val < 0:
                    dp[k][j] = max(dp[k][j], dp[k-1][j-1] + 0)
        
        # Process remaining rows
        for i in range(1, m):
            new_dp = [[-float('inf')] * n for _ in range(3)]
            ...
            for j in range(n):
                ...
                for k in range(3):
                    # Come from top
                    if k == 0:
                        new_dp[k][j] = max(new_dp[k][j], dp[k][j] + val)
                    else:
                        # Don't neutralize
                        new_dp[k][j] = max(new_dp[k][j], dp[k][j] + val)
                        # Neutralize
                        if val < 0:
                            new_dp[k][j] = max(new_dp[k][j], dp[k-1][j] + 0)
                    
                    # Come from left
                    if j > 0:
                        if k == 0:
                            new_dp[k][j] = max(new_dp[k][j], new_dp[k][j-1] + val)
                        else:
                            # Don't neutralize
                            new_dp[k][j] = max(new_dp[k][j], new_dp[k][j-1] + val)
                            # Neutralize
                            if val < 0:
                                new_dp[k][j] = max(new_dp[k][j], new_dp[k-1][j-1] + 0)
            
            dp = new_dp
        
        return max(dp[k][n-1] for k in range(3))

class Solution:
    def minimumDistance(self, nums: List[int]) -> int:
        # Store last 3 indices seen for each value
        last_three = defaultdict(list)  # num -> list of last 3 indices
        min_dist = float('inf')
        
        for i, num in enumerate(nums):
            last_three[num].append(i)
            # Keep only last 3
            if len(last_three[num]) > 3:
                last_three[num].pop(0)
            
            # Check if we have 3 occurrences
            if len(last_three[num]) == 3:
                span = last_three[num][2] - last_three[num][0]

class Solution:
    def largestOverlap(self, img1: List[List[int]], img2: List[List[int]]) -> int:
        n = len(img1)
        
        # Convert each row into a bitmask integer
        mask1 = [0] * n
        mask2 = [0] * n
        for i in range(n):
            for j in range(n):
                mask1[i] = (mask1[i] << 1) | img1[i][j]
                mask2[i] = (mask2[i] << 1) | img2[i][j]
        
        max_overlap = 0
        
        # Try all row shifts (dr) and column shifts (dc)
        for dr in range(-n + 1, n):
            for dc in range(-n + 1, n):
                current_overlap = 0
                
                # Compare overlapping rows
                for r1 in range(n):
                    r2 = r1 + dr
                    if 0 <= r2 < n:
                        # Shift row of img1 horizontally
                        if dc >= 0:
                            shifted_row = mask1[r1] >> dc
                        else:
                            shifted_row = mask1[r1] << abs(dc)
                        
                        # Count overlapping 1s using bitwise AND
                        current_overlap += (shifted_row & mask2[r2]).bit_count()
                
                max_overlap = max(max_overlap, current_overlap)
                
        return max_overlap




# from collections import Counter
# from typing import List

# class Solution:
#     def largestOverlap(self, img1: List[List[int]], img2: List[List[int]]) -> int:
#         n = len(img1)
        
#         # Store the coordinates of 1s for both images
#         ones1 = [(r, c) for r in range(n) for c in range(n) if img1[r][c] == 1]
#         ones2 = [(r, c) for r in range(n) for c in range(n) if img2[r][c] == 1]
        
#         # Count the frequency of each translation vector
#         offset_counts = Counter()
#         for r1, c1 in ones1:
#             for r2, c2 in ones2:
#                 offset_counts[(r2 - r1, c2 - c1)] += 1
                
#         # Return the maximum overlap found (0 if no ones overlap)
#         return max(offset_counts.values()) if offset_counts else 0
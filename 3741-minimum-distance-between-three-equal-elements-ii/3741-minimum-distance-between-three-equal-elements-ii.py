class Solution:
    def minimumDistance(self, nums: List[int]) -> int:
        # Track last two occurrences using dict of tuples
        # For values with only 1 occurrence so far, store (idx, -1)
        # For values with 2+ occurrences, store (second_last, last)
        last_two = {}
        min_dist = float('inf')
        
        for i, x in enumerate(nums):
            if x in last_two:
                first, second = last_two[x]
                if second != -1:
                    # Have 2 previous occurrences, form triple
                    min_dist = min(min_dist, 2 * (i - first))
                    last_two[x] = (second, i)
                else:
                    # Only 1 previous occurrence
                    last_two[x] = (first, i)
            else:
                last_two[x] = (i, -1)

class Solution:
    def sumAndMultiply(self, s: str, queries: List[List[int]]) -> List[int]:
        ...
        # Convert string to a list of digit integers once
        digits = [ord(c) - 48 for c in s]
        
        # Unified prefix arrays
        pref_x = [0] * (n + 1)
        pref_cnt = [0] * (n + 1)
        pref_sum = [0] * (n + 1)
        
        # Local variable caching for faster loop execution
        px, pc, ps = 0, 0, 0
        ...
        # Use a list comprehension to eliminate loop overhead and `.append()` speed penalties
        return [
            ((pref_x[r+1] - pref_x[l] * pow10[pref_cnt[r+1] - pref_cnt[l]]) % MOD * (pref_sum[r+1] - pref_sum[l])) % MOD
            for l, r in queries
        ]

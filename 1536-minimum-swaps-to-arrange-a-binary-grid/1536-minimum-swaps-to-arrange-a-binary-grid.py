class Solution:
    def minSwaps(self, grid: List[List[int]]) -> int:
        ...
        # For each row, find the position of the rightmost 1
        rightmost_one = []
        for i in range(n):
            pos = -1
        ...
        # Process each row
        for i in range(n):
            # For row i, we need all zeros above diagonal
            # So the rightmost 1 must be at or before column i-1
            # Actually, we need all cells above diagonal to be zero
            # For row i, columns i+1 to n-1 must be zero
            # So the rightmost 1 must be at or before column i
            required_pos = i
            
            # Find a row with rightmost one <= required_pos
            found = -1
            for j in range(i, n):
                if rightmost_one[j] <= required_pos:
                    found = j
                    break
            
            # If no row found, impossible
            if found == -1:
                return -1
